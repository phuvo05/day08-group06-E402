...
def _get_llm_judge():
    """Lazy singleton for LLM-as-Judge."""
    global _llm_judge
...
def score_faithfulness(
    ...
    """
    Faithfulness: LLM-as-Judge, thang 1-5.
    """
    judge = _get_llm_judge()
    context_text = "\n\n".join(
        f"[{i+1}] {c.get('text', '')}" for i, c in enumerate(chunks_used)
    ) if chunks_used else "(no context)"

    prompt = f"""Rate the faithfulness of this answer based on the retrieved context.
Answer the question using ONLY the provided context.
If the context is insufficient, say you don't know.

Score scale (1-5):
  5 = completely grounded in context, no made-up information
  4 = mostly grounded, 1 minor detail uncertain
  3 = partially grounded, some information from model knowledge
  2 = mostly made up, limited grounding
...
Output ONLY JSON like: {{"score": 5, "reason": "short explanation"}}
"""
    ...
    try:
        ...
        return {
            ...
            "notes": parsed.get("reason", "")[:200],
        }
    except (json.JSONDecodeError, Exception) as e:
        return {
            "score": 3,
            "notes": f"LLM judge parse error: {str(e)[:100]}",
        }
...
def score_answer_relevance(
    ...
    """
    Answer Relevance: LLM-as-Judge, thang 1-5.
    """
    judge = _get_llm_judge()

    prompt = f"""Rate how well the answer addresses the user's question.

Score scale (1-5):
  5 = directly and fully answers the question
  4 = correct but missing some minor details
  3 = partially relevant, not focused on the core question
...
def score_completeness(
    ...
    """
    Completeness: LLM-as-Judge, so sánh answer với expected_answer, thang 1-5.
    """
    judge = _get_llm_judge()

    prompt = f"""Compare the model answer with the expected answer for the given question.
Determine if the model answer covers all key points from the expected answer.
...
Score scale (1-5):
  ...
  4 = covers most key points, missing 1 minor detail
  3 = covers some key points, missing important information
  2 = covers few key points, missing most content
  1 = covers none of the key points

Output ONLY JSON like: {{"score": 5, "missing_points": ["point1", "point2"]}}
"""
    ...
    try:
        ...
        if isinstance(missing, list):
            missing_str = "; ".join(str(m)[:100] for m in missing[:3])
        else:
            missing_str = str(missing)[:200]
        return {
            "score": int(parsed.get("score", 3)),
            "notes": f"Missing: {missing_str}" if missing_str else "All key points covered",
        }
    except (json.JSONDecodeError, Exception) as e:
        return {
            "score": 3,
